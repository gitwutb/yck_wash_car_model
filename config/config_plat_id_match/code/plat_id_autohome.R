...
#读取数据
library(RMySQL)
#local_file<-gsub("\\/config.*","",dirname(rstudioapi::getActiveDocumentContext()$path))
...
source(paste0(local_file,"\\config\\config_plat_id_match\\config_function\\id_match.R"),echo=TRUE,encoding="utf-8")
data_input<-qx_czb
##调用函数计算结果列表
che3001<-che300
...
write.csv(result_output,paste0(local_file,"\\config\\config_plat_id_match\\out_autohome.csv"),row.names = F)