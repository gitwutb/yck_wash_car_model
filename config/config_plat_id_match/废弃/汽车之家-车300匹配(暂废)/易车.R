...
yck_czb<-dbFetch(dbSendQuery(loc_channel,"SELECT model_yr model_year,model_id car_id,brand_name,series_name,model_name model_name_t FROM config_yiche_major_info a;"),-1)
...
linshi<-gsub(c(str_c(1990:2030,"款",sep="",collapse='|')),"",yck_czb$model_name)
yck_czb<-data.frame(yck_czb,model_name=paste0(yck_czb$brand_name,series_all,linshi,sep=""))
#######################################
rm_rule<- read.csv("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_file\\reg_rule.csv",header = T,sep = ",")
rm_series_rule<- read.csv("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_file\\reg_series_rule.csv",header = T,sep = ",")
out_rrc<- read.csv("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_file\\out_rrc.csv",header = T,sep = ",")
che300<- read.csv("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_file\\qx_che300.csv",header = T,sep = ",")
source("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_fun\\fun_stopWords.R",echo=TRUE,encoding="utf-8")
source("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_fun\\fun_normalization.R",echo=TRUE,encoding="utf-8")
yck_czb<-data.frame(yck_czb,car_auto="",discharge_standard="",liter="",model_price="")
...
#########################################################################################################
##################################################第二大章：数据匹配#####################################
source("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_fun\\fun_match.R",echo=TRUE,encoding="utf-8")
source("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_fun\\fun_iteration.R",echo=TRUE,encoding="utf-8")
source("E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\config\\config_fun\\fun_match_result_czb.R",echo=TRUE,encoding="utf-8")
data_input<-qx_czb
##调用函数计算结果列表
...
write.csv(return_db,"匹配结果.csv",row.names = F)
write.table(data.frame(platform=unique(return_db$car_platform),accurate=round(accurate[2]/(accurate[1]+accurate[2]),3),n_right=nrow(match_right),n_repeat=nrow(match_repeat),n_not=nrow(match_not)),
            "E:\\Work_table\\Study\\Rexam\\YCK\\wash_car_model\\file\\output\\rizhi.txt",row.names = F,append=T)