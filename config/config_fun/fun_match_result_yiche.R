fun_match_result_yiche<-function(che300,data_input){
  qx_che300<-che300
  ...
  ################第五部分：同价位车匹配#####################
  ###########----------------同价位-----------######
  paste_yiche<-data.frame(id_data_input=qx_data_input$X,rname=qx_data_input$car_model_name,paste_yiche=paste(qx_data_input$car_year,qx_data_input$car_name,qx_data_input$car_series1,qx_data_input$car_price,sep = ""))
  paste_rrc<-data.frame(out_rrc,paste_rrc=paste(out_rrc$car_year,out_rrc$car_name,out_rrc$car_series1,out_rrc$car_price,sep = ""))
  
  paste_rrc$paste_rrc<-as.character(gsub(" ","",paste_rrc$paste_rrc))
  paste_yiche$paste_yiche<-as.character(gsub(" ","",paste_yiche$paste_yiche))
  linshi_wutb<-inner_join(paste_yiche,paste_rrc,c("paste_yiche"="paste_rrc"))%>%dplyr::select(x=paste_yiche,id_data_input,rname,id_che300,cname)
