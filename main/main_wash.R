...
price_model_loc<-c("E:/Work_table/gitwutb/git_project/yck_wash_car_model")
...
for (i in 1:length(car_platform)) {
  tryCatch({source(paste0(paste0("E:/Work_table/gitwutb/git_project/yck_wash_car_model","/main/main_detail/",sep=""),car_platform[i],sep=""),echo=TRUE,encoding="utf-8")},
           error=function(e){cat(write.table(data.frame(platform=paste0('iserror'),data=Sys.Date()),
                                         paste0("E:/Work_table/gitwutb/git_project/yck_wash_car_model","/file/output/rizhi_error.txt",sep=""),col.names = F,row.names = F,append=T),conditionMessage(e),"\n\n")},
...
library(RMySQL)
##拼接
files_full <- list.files(paste0("E:/Work_table/gitwutb/git_project/yck_wash_car_model","/file/output",sep=""), full.names = T,pattern = "csv")
...
write.csv(output_final,paste0("E:/Work_table/gitwutb/git_project/yck_wash_car_model","/file/output_final/output.csv",sep=""),
...
if(length(files_full)>=3){
  ...
  dbSendQuery(loc_channel,paste0("LOAD DATA LOCAL INFILE '","E:/Work_table/gitwutb/git_project/yck_wash_car_model","/file/output_final/output.csv'",
  ...
  write.table(paste0("match表 ",Sys.Date(),"   成功！"),
              paste0("E:/Work_table/gitwutb/git_project/yck_wash_car_model","/file/output/suc_or_los.txt",sep=""),row.names = F,col.names=F,append=T)
}else{
