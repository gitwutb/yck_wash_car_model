...
write.csv(wutb,paste0(local_file,"/file/output_final/analysis_wide_table.csv",sep=""),
          row.names = F,fileEncoding = "UTF-8",quote = F)

##日志文件
n_platform<-data.frame(n_platform,date=Sys.time())
write.table(n_platform,paste0(local_file,"/file/output_final/analysis_wide_table.txt",sep=""),row.names = F,append = T)
