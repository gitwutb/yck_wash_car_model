...
yck_yiche<-dbFetch(dbSendQuery(loc_channel,"SELECT id car_id,brand brand_name,series series_name,model model_name,msrp model_price FROM spider_www_yiche a 
                               WHERE a.id>(SELECT MAX(id_data_input) FROM analysis_match_id WHERE car_platform in ('yiche'));"),-1)
...
write.csv(return_db,paste0(deep_local,"yck_wash_car_model\\file\\output\\yiche.csv",sep=""),row.names = F)
#########################################################################
###日志文件
