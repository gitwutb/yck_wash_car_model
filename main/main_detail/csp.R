...
yck_csp<-dbFetch(dbSendQuery(loc_channel,"SELECT id car_id,model model_name,emission discharge_standard,gearbox car_auto FROM spider_www_chesupai a
                             WHERE a.id>(SELECT MAX(id_data_input) FROM analysis_match_id WHERE car_platform in ('csp'));"),-1)
...
##调用函数计算结果列表
list_result<-fun_match_result(che300,qx_csp)
...
return_db<-data.frame(car_platform="csp",return_db)
return_db$id_che300<-as.integer(as.character(return_db$id_che300))
write.csv(return_db,paste0(deep_local,"yck_wash_car_model\\file\\output\\csp.csv",sep=""),row.names = F)
###日志文件
