...
yck_guazi<-dbFetch(dbSendQuery(loc_channel,"SELECT id car_id,model model_name,emission discharge_standard,gearbox car_auto FROM spider_www_guazi a
                               WHERE a.id>(SELECT MAX(id_data_input) FROM analysis_match_id WHERE car_platform in ('guazi'));"),-1)
...
write.csv(return_db,paste0(deep_local,"yck_wash_car_model\\file\\output\\guazi.csv",sep=""),row.names = F)
###日志文件
